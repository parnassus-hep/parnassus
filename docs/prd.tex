...
To demonstrate the framework in practice, we present results from running
Parnassus on 100 $H \to ZZ \to 4\ell$ events from a HepMC input file, using the
bundled \texttt{cms\_2011\_flow\_v00} model with 50 Euler diffusion steps on CPU.
Total processing time was approximately 3 minutes.

\subsection{Per-Event Output}

Table~\ref{tab:perevt} shows the per-event truth and pflow particle counts and
$H_\mathrm{T}$ for the first 10 events.

\begin{table}[H]
\centering
\caption{Per-event summary for the first 10 generated events ($H \to ZZ \to 4\ell$,
  HepMC input, 50 diffusion steps).}
\label{tab:perevt}
\begin{tabular}{@{}rrrrr@{}}
\toprule
\textbf{Event} & \textbf{$n_\mathrm{truth}$} & \textbf{$n_\mathrm{pflow}$}
  & \textbf{Truth $H_\mathrm{T}$ (GeV)} & \textbf{Pflow $H_\mathrm{T}$ (GeV)} \\
\midrule
...
Table~\ref{tab:aggregate} summarises the aggregate statistics over all 100 events.

\begin{table}[H]
\centering
\caption{Aggregate statistics over 100 $H \to ZZ \to 4\ell$ events.}
\label{tab:aggregate}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Quantity} & \textbf{Truth} & \textbf{Pflow} \\
\midrule
Particles per event        & $96.4 \pm 46.4$   & $47.4 \pm 48.5$ \\
$H_\mathrm{T}$ (GeV)      & $793.6 \pm 171.2$ & $1375.4 \pm 1614.1$ \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:classdist} compares the particle class composition between truth and
generated pflow particles across all 100 events.

\begin{table}[H]
\centering
\caption{Particle class distribution across 100 events (9\,644 truth particles,
  4\,742 pflow particles).}
\label{tab:classdist}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Class} & \textbf{Truth} & \textbf{\%} & \textbf{Pflow} & \textbf{\%} \\
\midrule
...
\midrule
\textbf{Total} & \textbf{9\,644} & & \textbf{4\,742} & \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:jets} summarises the jet reconstruction results for truth and pflow
collections using the anti-$k_t$ algorithm with $R = 0.5$.

\begin{table}[H]
\centering
\caption{Jet reconstruction summary (anti-$k_t$, $R = 0.5$, $p_\mathrm{T}^\mathrm{min} = 10$\,GeV).}
\label{tab:jets}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Collection} & \textbf{Jets/event} & \textbf{Mean $p_\mathrm{T}$ (GeV)}
  & \textbf{Median $p_\mathrm{T}$ (GeV)} & \textbf{Max $p_\mathrm{T}$ (GeV)} \\
\midrule
Truth anti-$k_t$ $R{=}0.5$ & $20.8 \pm 9.4$  & 31.5  & 10.6  & 648.1  \\
Pflow anti-$k_t$ $R{=}0.5$ & $10.9 \pm 11.1$ & 100.3 & 39.3  & 1875.1 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:leptons} shows the lepton reconstruction and isolation results.
The $H \to ZZ \to 4\ell$ process provides a clean test case with
well-identified electrons and muons.

\begin{table}[H]
\centering
\caption{Reconstructed lepton summary (100 events, isolation cone $\Delta R = 0.4$).}
\label{tab:leptons}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Lepton} & \textbf{Per event} & \textbf{Mean $p_\mathrm{T}$ (GeV)}
  & \textbf{Mean iso.} & \textbf{Median iso.} \\
\midrule
...
Table~\ref{tab:impact} shows the distributions across all generated pflow particles.

\begin{table}[H]
\centering
\caption{Impact parameter distributions across all pflow particles (100 events).}
\label{tab:impact}
\begin{tabular}{@{}lrr@{}}
\toprule
...
\subsection{Pythia8 Input}

We also ran the same pre-trained model on events generated on-the-fly by
\textsc{Pythia8} using the bundled \texttt{HZZ4l.cmnd} steering card. Of
20 requested events, 6 passed quality filters (the VBF $H \to ZZ \to 4\ell$
process has a small cross section, and many events exceed the 400-particle
limit). Table~\ref{tab:pythia} shows the results.

\begin{table}[H]
\centering
\caption{Per-event summary from \textsc{Pythia8} input (VBF $H \to ZZ \to 4\ell$,
  6 events passing quality cuts).}
\label{tab:pythia}
\begin{tabular}{@{}rrrrr@{}}
\toprule
\textbf{Event} & \textbf{$n_\mathrm{truth}$} & \textbf{$n_\mathrm{pflow}$}
...
The generated ROOT file contains 67 branches organised into collections.
The output for 100 events occupies approximately 486\,KB on disk.

\begin{lstlisting}[style=bash,caption={Reading the output ROOT file with \texttt{uproot}.},label=lst:readroot]
import uproot
import numpy as np
...
#   Truth, Pflow, Electrons, Muons,
#   TruthJetsAntiKt05, TruthJetsAntiKt08,
#   PflowJetsAntiKt05
