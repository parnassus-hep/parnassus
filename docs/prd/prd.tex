...
behaviour, we present results from running Parnassus on three different physics
processes using the bundled \texttt{cms\_2011\_flow\_v00} model with 50 Euler
diffusion steps on CPU:
...
400-particle limit. All results use the default configuration with anti-$k_t$
...
across all three physics processes. The $p_\mathrm{T}$ spectra follow the
...
distributions reflect the different underlying event structures.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/kinematic_distributions.pdf}
\caption{Particle-flow kinematic distributions across three physics processes.
...
broadly consistent across processes.
...
events from all three processes, highlighting the different event topologies
and demonstrating the model's ability to handle diverse physics signatures.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/process_comparison.pdf}
\caption{Side-by-side comparison of Parnassus pflow output for three physics
  processes. Each panel shows one representative event in $\eta$--$\phi$ with
  particle classes colour-coded and anti-$k_t$ jets overlaid as dashed circles.
  The distinct event topologies --- low-multiplicity leptonic ($H \to 4\ell$),
  hadronic dijet (QCD), and electroweak (Drell--Yan) --- are all produced by
...
\begin{table}[H]
\centering
\caption{Aggregate statistics across three physics processes (mean $\pm$ std).}
\label{tab:aggregate}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Quantity}
  & \textbf{$H{\to}4\ell$ (100\,evt)}
  & \textbf{QCD dijet (832\,evt)}
  & \textbf{Drell--Yan (555\,evt)} \\
\midrule
Truth particles/event  & $96.4 \pm 46.4$   & $107.1 \pm 45.9$  & $76.1 \pm 39.3$ \\
Pflow particles/event  & $47.4 \pm 48.5$   & $43.2 \pm 39.4$   & $42.3 \pm 36.5$ \\
Truth $H_\mathrm{T}$ (GeV) & $793.6 \pm 171.2$ & $775.1 \pm 166.8$ & $786.4 \pm 137.4$ \\
Pflow $H_\mathrm{T}$ (GeV) & $1375 \pm 1614$   & $1262 \pm 1680$   & $1187 \pm 1486$ \\
\bottomrule
\end{tabular}
\end{table}
